...
public class os {
	...
	public static void Crint(int[]a, int[]p){
		...
		if (addressTable.assignJob(newestJob)){																						//2a. addressTable checks if there's enough free space. If there is it gets allocated free space and put on the readyqueue
			sos.siodrum(newestJob.getJobNumber(), newestJob.getJobSize(), newestJob.getJobAddress(), 1);		//3a. Don't know if I should do this with siodrum. Puts job on core (memory)
			...
			sos.siodrum(newestJob.getJobNumber(), newestJob.getJobSize(), newestJob.getJobAddress(), 0);		//3b. Don't know if I should do this with siodrum. Puts job on backing store
		}
